...
class VisualBiasLoss(nn.Module):
    ...
    def transfer_xyz(self, gt, pd, valid_mask, batch_size):
        ...
        x_image = torch.unsqueeze(u_u0, dim=0).repeat(batch_size, 1, 1, 1) / self.fx #image coor
        y_image = torch.unsqueeze(v_v0, dim=0).repeat(batch_size, 1, 1, 1) / self.fy #image coor

        x_image = torch.flatten(x_image, start_dim=2)[valid_mask]
        y_image = torch.flatten(y_image, start_dim=2)[valid_mask]



        # print('repeat u_uo', u_u0.shape)

        z = self.get_z(x_image, y_image, gt)
        z_pd = self.get_z(x_image, y_image, pd)

        x = x_image * z #gt point cloud x 
        y = y_image * z #gt point cloud y
        ...
        z = torch.unsqueeze(z_pd, dim=0)

        pcd = torch.cat((x, y, z), 0)
        return pcd
        # return pcd, valid_mask, valid_flatten_gt

    def transfer_xyz_old(self, gt, pd, valid_mask, batch_size):
        '''
        pixel to world coordinate
        and use pd to z
        '''
    ...
    def cal_distance(self, pd, valid_mask, norm_level):
        gt_u = self.u[valid_mask]
        gt_v = self.v[valid_mask]

        gt_u_0 = torch.abs(self.gt_u_u0[valid_mask]) + norm_level #1e-5
        gt_v_0 = torch.abs(self.gt_v_v0[valid_mask]) + norm_level
    ...
    def bias_point_to_uv(self, point_cloud, valid_mask, norm_level):
        ...
        distance = self.cal_distance(pd_uv_list, valid_mask, norm_level)
    ...
    def VBloss(self, input, target, valid_mask, batch_size, norm_level):
        ...
        # bias_point_cloud = self.transfer_xyz_old(target, input, valid_mask, batch_size) 
        vb_loss = self.bias_point_to_uv(bias_point_cloud, valid_mask, norm_level)
    ...
    def forward(self,
        ...
        vb = self.loss_weight * self.VBloss(depth_pred, depth_gt, valid_mask, batch_size, norm_level)
        # vb_d = self.loss_weight * self.VBloss(self.DepthNorm(depth_pred), self.DepthNorm(depth_gt), valid_mask, batch_size, norm_level)
