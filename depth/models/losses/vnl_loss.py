...
class VNL_Loss(nn.Module):
    ...
    def filter_mask(self, p123, gt_xyz, delta_cos=0.867,
        ...
        mask_pad = torch.sum(torch.logical_and(pw[:, :, 2, :] > self.delta_z, pw[:, :, 2, :] < 1.0), 2) == 3
    ...
    def VNL_loss(self, gt_depth, pred_depth, select=True):
        """
        Virtual normal loss.
        ...
        # print('vn, vnnormalize', gt_normal[0][0], gt_norm[0][0])
        gt_normal = gt_normal / gt_norm
        # print('norm vn', gt_normal[0][0])
    ...
    def forward(self,
        ...
        depth_gt = depth_gt / self.max_depth
        depth_pred = depth_pred / self.max_depth
        
        

        loss_depth = self.loss_weight * self.VNL_loss(depth_gt, depth_pred)
