...
class VisualBiasLoss(nn.Module):
    ...
    def cal_distance(self, pd, valid_mask):
        ...
        gt_u_0 = torch.abs(self.gt_u_u0[valid_mask]) +0.001
        gt_v_0 = torch.abs(self.gt_v_v0[valid_mask]) +0.001
    ...
    def VBloss(self, input, target, valid_mask, batch_size):
        ...
        # return (torch.sqrt(Dg) * 10.0 + vb_loss * 10.0) * 0.5


        return vb_loss * 5.0

        # return torch.log10(loss)
    def DepthNorm(self, depth, maxDepth=10000.0):  #1000*10
    ...
    def forward(self,
        ...
        u_non_zero = self.gt_u_u0 != 0
        v_non_zero = self.gt_v_v0 != 0
        non_zero_mask = torch.logical_and(u_non_zero, v_non_zero)
