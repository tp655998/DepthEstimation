...
class VNL_Loss(nn.Module):
    ...
    def __init__(self,
                 ...
                 input_size=(416, 544),
                 delta_cos=0.867, 
                 delta_diff_x=0.007*80.0,
                 delta_diff_y=0.007*80.0, 
                 sample_groups=5000, 
                 xyz_mode = 'uvd',
                 loss_weight=1.0,
                 max_depth=None):
    ...
    def VNL_loss(self, gt_depth, pred_depth, mask):
        """
        Virtual normal loss.
        ...
        for i in range(B):
            ...
            # print('pred_depth_i', pred_depth_i.shape)
            mask_i = mask[i, :][None, :, :]
            ...
            if len(planes) == 0:
                # print('Empty')
                continue
            # mask_planes = torch.stack(planes, dim=0) 
            mask_planes = torch.cat(planes, dim=0) #
            # print('mask_planes', mask_planes.shape)
    ...
    def forward(self,
        ...
        mask_kp1 = depth_gt > 70.0
        mask_kp2 = depth_gt < 40.0
        mask = 1 * mask_kp1 + 2 * mask_kp2


        loss_depth = self.loss_weight * self.VNL_loss(depth_gt, depth_pred, mask)
