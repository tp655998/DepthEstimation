...
class HAHIHeteroNeck(BaseModule):
    ...
    def forward(self, inputs):
        ...
        # print('feats_trans:', len(feats_trans)) #select 4 swin transformer layers
        # print('src_flattens start:', len(src_flattens)) # 0

        for i in range(len(feats_trans)):
            bs, c, h, w = feats_trans[i].shape
            # print('i = {} c = {} h = {} w = {}'.format(i, c, h, w))
            # i = 0 c = 96 h = 104 w = 136
            # src_flattens 0 torch.Size([2, 14144, 256])

            # i = 1 c = 192 h = 52 w = 68
            # src_flattens 1 torch.Size([2, 3536, 256])

            # i = 2 c = 384 h = 26 w = 34
            # src_flattens 2 torch.Size([2, 884, 256])

            # i = 3 c = 768 h = 13 w = 17
            # src_flattens 3 torch.Size([2, 221, 256])
            ...
            # print('src_flattens', i, len(src_flattens))
            # print('src_flattens', i, np.shape(src_flattens[i]))


        # print('src_flattens', i, len(src_flattens)) # 4
        src_flatten = torch.cat(src_flattens, 1)

        # print('src_flatten cat', src_flatten.shape)
        # src_flatten cat torch.Size([2, 18785, 256])
        ...
        # HI (deformable cross attention)===============================
        # CNN Branch
        conv_skip = self.conv_proj(feat_conv)
        bs, c, h, w = conv_skip.shape
        # print('Resnet: c = {} h = {} w = {}'.format(c, h, w))
        # Resnet: c = 256 h = 208 w = 272
        ...
        # print('Resnet query_embed', query_embed.shape)
        # Resnet query_embed torch.Size([2, 56576, 256])
        ...
        # print('Resnet enhence', fusion_res_conv.shape)
