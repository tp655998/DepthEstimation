...
class BTSHead(DepthBaseDecodeHead):
    ...
    def forward_train(self, img, inputs, img_metas, depth_gt, train_cfg):
        ...
        # losses["loss_mse"] = self.loss_mse(depth_pred, depth_gt)
        # losses["loss_rmsle"] = self.loss_rmsle(depth_pred, depth_gt)
        #losses["loss_vb"] = self.loss_vb(img, depth_pred, depth_gt) #=============================
